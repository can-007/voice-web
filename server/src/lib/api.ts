...
export default class API {
  ...
  getRouter(): Router {
    ...
    router.get('/challenge/points/', this.getChallengePoint);
    router.get('/challenge/weekly/:date', this.getWeeklyChallenge);
    router.get('/:locale/top/member/:type', this.getTopMembers);
    router.get('/:locale/top/teams/:type', this.getTopTeams);
    router.get('/:locale/top/contributors/:type', this.getTopContributors);
  ...
  getUserClients = async ({ client_id, user }: Request, response: Response) => {
    ...
    const challenge_team = user.challenge_team;
    const userClients: UserClientType[] = [
      { email, challenge_team },
  ...
  getChallengePoint = async ({ client_id }: Request, response: Response) => {
    console.log(`[DEBUG] getChallengePoint - client_id:${client_id}`);
    let data = null;
    try {
      let mock_path =
        process.env.SERVER_CONFIG_PATH || './challenge-mock-data.json';
      data = JSON.parse(fs.readFileSync(mock_path, 'utf-8'));
    } catch (err) {
  ...
  getWeeklyChallenge = async (
    ...
    console.log(
      `[DEBUG] getWeeklyChallenge - client_id:${client_id}, date:${JSON.stringify(
        date
      )}`
    );
    let data = null;
  ...
  getTopMembers = async (
    ...
    console.log(
      `[DEBUG] getTeamProgress - locale: ${locale}, client_id:${client_id}, type:${type}`
    );
    let data = null;
    try {
      let mock_path =
  ...
  getTopTeams = async (
    ...
    console.log(
      `[DEBUG] getTopTeams - client_id:${client_id}, locale: ${locale}, type:${type}`
    );
    let data = null;
    try {
      let mock_path =
  ...
  getTopContributors = async (
    ...
    console.log(
      `[DEBUG] getTopContributors - client_id:${client_id} locale: ${locale}, type:${type}`
    );
    let data = null;
    try {
      let mock_path =
